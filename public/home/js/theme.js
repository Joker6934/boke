...
$('.like').click(function(){
	...
	if(_this.attr('uid')&&_this.hasClass('like-no')){
		...
		$.ajax({
			...
			success: function(){
				...
				_this.addClass("like-animate").attr("title","已喜欢");
				setTimeout(function(){_this.removeClass('like-animate').removeClass('like-no').addClass('like-yes');},500);
			}});
		return false;
	}else if(_this.attr('uid')&&_this.hasClass('like-yes')){
		var uid = Number(_this.attr('uid'));
		$.ajax({
			type: 'POST',
			dataType: 'html',
			...
			cache: false,
			success: function(){
				_this.children("span").text("点击喜欢");
				_this.addClass("like-animate").attr("title","点击喜欢");
				setTimeout(function(){_this.removeClass('like-animate').removeClass('remove-like').removeClass('like-yes').addClass('like-no');},500);
			}
		});
		return false;
	}else{
...
$('#submit-mail').click(function(){
	...
	function mail_success(){
		errTimer = setTimeout(function(){
			$('#mailmessage').html('<p class="mail-success"><i class="fa fa-check"></i>消息发送成功</p><div><img src="'+tin.tin_url+'/images/weixin.png"></div>');
		},2000)
	}
    return false;
})
