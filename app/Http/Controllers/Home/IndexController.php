...
//            喜欢操作
            case 'add':
                ...
                if($like->isEmpty()){
                    //添加喜欢记录
                    $res = Like::create(['uid'=>$uid,'art_id'=>$artid]);
                    Article::where('art_id',$artid)->increment('art_like');
                    //如果喜欢成功
                    if($res){
                        return response()->json(['status'=>0,'msg'=>'已喜欢']);
                    }else{
                        return response()->json(['status'=>1,'msg'=>'喜欢失败']);
                    }
                }else{
                    return response()->json(['status'=>0,'msg'=>'已喜欢']);
                }
            ...
            case 'remove':
                ...
                if(!empty($like)) {
                    //文章的喜欢数量-1
                    Article::where('art_id',$artid)->decrement('art_like');
//                    取消喜欢
                    $res = $like->delete();
                    if($res){
                        return response()->json(['status'=>0,'msg'=>'请喜欢']);
                    }else{
                        return response()->json(['status'=>0,'msg'=>'取消喜欢失败']);
                    }
                }else{
                    return response()->json(['status'=>0,'msg'=>'请喜欢']);
                }
